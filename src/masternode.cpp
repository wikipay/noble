...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if(Params().NetworkID() == CBaseChainParams::MAIN) {
        if(addr.GetPort() != 9449) return false;
    } else if(addr.GetPort() == 9449) return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 NOBLE tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    ...
    {
        CBlockIndex* pMNIndex = (*mi).second; // block for 1000 NOBLE tx -> 1 confirmation
