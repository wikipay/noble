...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("Noble Core noble-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  noble-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded noble transaction") + "\n" +
              "  noble-tx [options] -create [commands]   " + _("Create hex-encoded noble transaction") + "\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded noble transaction
